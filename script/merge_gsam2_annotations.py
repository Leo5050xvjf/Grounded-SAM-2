...
#             # copy meta fields we want to keep
#             base_meta = {
#                 "frame_index": data.get("frame_index"),
#                 "box_format": data.get("box_format", "xyxy"),
#                 "img_width": data.get("img_width"),
#                 "img_height": data.get("img_height"),
#                 # keep original path as a reference, but we'll also write 'source_image'
#                 "image_path": data.get("image_path"),
#             }
#         anns = data.get("annotations", [])
#         # 過濾掉 None/空 segmentation 的標註（保險）
#         clean_anns = []
#         for a in anns:
#             if not isinstance(a, dict):
#                 continue
#             # 標準化: 確保 class_name、bbox、segmentation 存在
#             if "class_name" in a and "bbox" in a:
#                 clean_anns.append(a)
#         annotations_all.extend(clean_anns)

#     if base_meta is None:
#         return None

#     merged = dict(base_meta)
#     merged["annotations"] = annotations_all
#     return merged
...
#     # Collect per-class frames_json folders
#     class_json_dirs = []
...
#     # Choose a reference class (first one that has files) to enumerate frame names
#     ref_files = []
...
#     # Prepare pycocotools for mask decoding
...
#         # gather same-name json from each class dir
#         same_name_paths = []
#         for cj in class_json_dirs:
...
#         # Override image_path to the new vis path; also keep source image
...
#         # draw each annotation (mask first, then bbox + label)
#         for ann in merged.get("annotations", []):
#             cls = ann.get("class_name", "object")
#             color = color_for_class(cls)
#             score = ann.get("score", None)
#             label = f"{cls}" + (f" {score:.2f}" if isinstance(score, (int, float)) else "")

#             # draw mask if segmentation present
...
#                     }
#                     m = maskUtils.decode(rle)  # HxW, uint8 {0,1}
#                     # 如果尺寸不一致，做 resize（保守做法是最近鄰）
